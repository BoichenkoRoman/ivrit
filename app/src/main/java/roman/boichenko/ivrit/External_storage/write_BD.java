...
//                    }
    ...
    public void getListAbc() {
        ...
        abcd_arr.add(abc);
        abc = new Abc(12, "Каф/Хаф софит (конечная)", "ך", "@drawable/kaf-sofit", "«Каф/хаф софит пишется тогда, когда является последней в слове. Визуально отличается добавлением опущенного вниз, длинного штриха.");
        abcd_arr.add(abc);
        ...
        abcd_arr.add(abc);
        abc = new Abc(15, "Мэм софит (конечная)", "ם", "@drawable/mem-sofit", "«Мэм софит» состоит из круга и небольшой палочки, которая примыкает к нему с левой стороны. Кончик может незначительно выступать за пределы строчки.");
        abcd_arr.add(abc);
        abc = new Abc(16, "Нун", "נ", "@drawable/nun", "Прописная «нун» пишется сверху вниз, чем-то напоминает клюшку.");
        abcd_arr.add(abc);
        abc = new Abc(17, "Нун софит (конечная)", "ן", "@drawable/nun-sofit", "«Нун софит» — длинная вертикальная палочка заметно выступающая за пределы строки.");
        abcd_arr.add(abc);
        ...
        abcd_arr.add(abc);
        abc = new Abc(21, "Пэй/Фэй софит (конечная)", "ף", "@drawable/pei-sofit", "«Пэй/фэй софит» по своему написанию напоминает «ламэд», но в отличии от нее верхний штрих изгибается петлёй и возвращается к основанию.");
        abcd_arr.add(abc);
        ...
        abcd_arr.add(abc);
        abc = new Abc(23, "Цадик софит (конечная)", "ץ", "@drawable/cadik-sofit", "«Цадик софит» легко спутать с «пэй софит». Единственное отличие — хвостик не загибается к основанию, а наоборот взмывает в высь.");
        abcd_arr.add(abc);
